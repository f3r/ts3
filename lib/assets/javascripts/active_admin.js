...
jQuery(function($) {
	if($("div.attributes_table.menu_section table tbody tr td table tbody").length) {
		$("div.attributes_table.menu_section table tbody tr td table tbody").disableSelection();
		return $("div.attributes_table.menu_section table tbody tr td table tbody").sortable({
			axis: 'y',
			cursor: 'move',
			update : function(event,ui){
				var formData = $("div.attributes_table.menu_section table tbody tr td table tbody").sortable('serialize');
				formData += "&" + $('meta[name=csrf-param]').attr("content") + "=" + encodeURIComponent($('meta[name=csrf-token]').attr("content"));
				return $.ajax({
				    type: 'post',
				    data: formData,
