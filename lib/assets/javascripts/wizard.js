...
var currentPanel = function() {
  return $('ul#wizard_tabs li a[class=active]').attr('href');
}

var switchToPanel = function(target) {
  $('ul#wizard_tabs li a[href="' + target + '"]').click();
}
...
var validatePlace = function(panelStatus) {
  ...
  if(validCategories == 5) {
    ...
    $('#listing-status').attr('href', '/listings/' + $('#place_id').val());

    // If not published, display publish instructions, otherwise unpublish instructions.
    if($('#place_published').val() == '0') {
...
var validatePanels = function(target) {
  ...
  if(panelErrors.general)   { wizard_aside.find('#generalInfo img').attr('src',   '/assets/check.png');          }
  else                      { wizard_aside.find('#generalInfo img').attr('src',   '/assets/check-disabled.png'); }
  if(panelErrors.photos)    { wizard_aside.find('#photoInfo img').attr('src',     '/assets/check.png');          }
  else                      { wizard_aside.find('#photoInfo img').attr('src',     '/assets/check-disabled.png'); }
  if(panelErrors.amenities) { wizard_aside.find('#amenitiesInfo img').attr('src', '/assets/check.png');          }
  else                      { wizard_aside.find('#amenitiesInfo img').attr('src', '/assets/check-disabled.png'); }
  if(panelErrors.price)     { wizard_aside.find('#priceInfo img').attr('src',     '/assets/check.png');          }
  else                      { wizard_aside.find('#priceInfo img').attr('src',     '/assets/check-disabled.png'); }
...
var panelStatuses = function() {
  ...
  $.ajax({
    ...
    async: false,
    cache: false,
    url: '/listings/' + $("#place_id").val() + '/publish_check.json',
...
var validatePreview = function() {
  ...
  if(_panelStatus.general && _panelStatus.photos && _panelStatus.amenities && _panelStatus.price && _panelStatus.calendar) {
    ...
    if(!_panelStatus.price)     {$('#priceInfo').validationEngine(    'showPrompt', 'Price & Conditions needed', 'load', 'centerLeft');}
...
Wizard = {
  initialize: function(){
    $('ul#wizard_tabs li a').die().bind('click', switchPanel);
    this.updateUrl = $('#wizard_form').attr('action');
  },
  sendFieldUpdate: function(){
    alert(Wizard.updateUrl);
  },
  bindFields: function(){
    //.change(trackChange)
    $('#wizard_form input[type=text].autosave, #wizard_form textarea.autosave, #wizard_form select.autosave').blur(this.sendFieldUpdate);
  }
...
if(false){
  ...
  // place_place_size
    ...
    // $('#place_place_size, #wizard_form select.autosave_place_unit').change(trackChange)
    //       .blur(sendFieldUpdate);
    ...
    $('ul#wizard_tabs li a').click(function(){
